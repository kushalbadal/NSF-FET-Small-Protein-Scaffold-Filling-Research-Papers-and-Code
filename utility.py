...
if __name__ == "__main__":
    ...
    cwd = os.getcwd()
    os.chdir(os.path.join(cwd, "newdatasets"))
    target = get_sequences("target_sequence.txt")[0]
    denovo = get_sequences("de_novo_sequence.txt")[0]

    correct_indices = np.array([])
    incorrect_indices = np.where(target != denovo)[0]

    os.chdir(os.path.join(cwd, "autoencoder_results"))
    write_protein_scaffold_image(target, np.array([]), np.array([]), "target_sequence")
    write_protein_scaffold_image(denovo, incorrect_indices, np.array([]), "de_novo_sequence")
